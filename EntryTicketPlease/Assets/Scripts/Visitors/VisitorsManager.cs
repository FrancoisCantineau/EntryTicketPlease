        ...
        if (visitorQueue.Count > 0)
        {
            if (currentVisitorIndex < visitorQueue.Count)
            {
                if (spawnPoint != null)
                {
                    ...
                    // Ajout des composants une seule fois
                    currentVisit.AddComponent<CharacterNavMeshMovement3D>();
                    currentVisit.AddComponent<NavMeshAgent>();

                    currentVisitor = currentVisit;

                    currentVisitorIndex++;
                    Debug.Log("Visitor spawned: " + currentVisitor.name + " at index " + (currentVisitorIndex - 1));
                }
            }
            else
            {
